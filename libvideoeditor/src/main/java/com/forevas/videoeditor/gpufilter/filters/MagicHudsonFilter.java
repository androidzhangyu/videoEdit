...
public class MagicHudsonFilter extends GPUImageFilter {
	...
	protected void onInitialized(){
	    ...
	    runOnDraw(new Runnable(){
		    public void run(){
		    	inputTextureHandles[0] = OpenGlUtils.loadTexture(VideoEditorSDK.getInstance().getContext(), "filter/hudsonbackground.png");
				inputTextureHandles[1] = OpenGlUtils.loadTexture(VideoEditorSDK.getInstance().getContext(), "filter/overlaymap.png");
				inputTextureHandles[2] = OpenGlUtils.loadTexture(VideoEditorSDK.getInstance().getContext(), "filter/hudsonmap.png");
		    }
	    });
	}
}
