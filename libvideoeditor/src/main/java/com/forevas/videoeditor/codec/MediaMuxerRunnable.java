...
public class MediaMuxerRunnable extends Thread {
    ...
    private AudioSettingInfo mAudioSettingInfo;
    ...
    public void setAudioSetting(AudioSettingInfo info) {
        mAudioSettingInfo = info;
    }
    ...
    public void run() {
        ...
        initMuxer();
        mAudioRunnable = new AudioRunnable(mVideoInfos, mAudioSettingInfo, this);
        mVideoRunnable = new VideoRunnable(mVideoInfos, mFilterType, this);
     ...
     */
    public void addMeidaFormat(int trackIndex, MediaFormat format) {
        synchronized (lock) {
            if (mMediaMuxer == null) {
                return;
            }
