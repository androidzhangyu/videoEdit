...
package com.forevas.videoeditor.encoder.video;
...
public class VideoEncoderCore {
    ...
    public void release() {
        ...
        System.out.println("MediaMux release");
        if (mMuxer != null) {
        ...
        while (true) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                synchronized (lock) {
                    ...
                    System.out.println("MediaMux addVideoTrack");
                    if (mVideoTrackIndex >= 0 && mAudioTrackIndex >= 0) {
                        mMuxer.start();
                        System.out.println("MediaMux start");
                        mMuxerStarted = true;
                    }
                }
...
//                    encodedData.limit(mBufferInfo.offset + mBufferInfo.size);
                    if(!mMuxerStarted){//双重校验锁,确保muxer没有启动再wait
                        synchronized (lock){
                            if(!mMuxerStarted){
                                try {
                                    lock.wait();
                    ...
                    if (mMuxerStarted) {
                        System.out.println("MediaMux addVideoData");
                ...
                if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    System.out.println("MediaMux videoEndOfStream");
                    if (!endOfStream) {
    ...
    class AudioEncoder implements Runnable {
        ...
        private boolean audioStep() throws IOException {
            int index = mAudioEnc.dequeueInputBuffer(TIMEOUT_USEC);
            if (index >= 0) {
            ...
            int outIndex;
            do {
                outIndex = mAudioEnc.dequeueOutputBuffer(mInfo, TIMEOUT_USEC);
                if (outIndex >= 0) {
                    if ((mInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                        Log.e(TAG, "audio end");
                        System.out.println("MediaMux audioEndOfStream");
                    ...
                    if (mMuxerStarted && mInfo.presentationTimeUs > 0) {
                        try {
                            System.out.println("MediaMux addAudioData");
                    ...
                    synchronized (lock) {
                        ...
                        System.out.println("MediaMux addAudioTrack");
                        Log.e(TAG, "add audio track-->" + mAudioTrackIndex);
                        if (mAudioTrackIndex >= 0 && mVideoTrackIndex >= 0) {
                            mMuxer.start();
                            System.out.println("MediaMux start");
                            mMuxerStarted = true;
                            lock.notify();//如果video线程正在等待,则唤醒
                        }
                    }
                }
            } while (outIndex >= 0);
